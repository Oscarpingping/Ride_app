...
export default function HomeScreen() {
  ...
  const { loading, error, getClubs } = useClubs();
  ...
  const filteredRides = useMemo(() => {
    return rides.filter(ride => {
      ...
      // Terrain filter
      if (filters.terrain.length > 0 && !filters.terrain.includes(ride.terrain as TerrainType)) {
        return false;
      }

      // Pace filter
      if (filters.pace.length > 0 && !filters.pace.includes(ride.pace as PaceLevel)) {
        return false;
      }

      // Difficulty filter
      if (filters.difficulty.length > 0 && !filters.difficulty.includes(ride.difficulty as DifficultyLevel)) {
        return false;
      }
